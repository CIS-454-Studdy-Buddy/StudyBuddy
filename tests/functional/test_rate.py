...
'''
This function tests if buddy rating is sucessfully completed and the rewards points and buddy star rating change
if you rate the same buddy for multiple months 
'''
@mock.patch("app.extensions.email.send", return_value=True, autospec=True)
@mock.patch("random.randint", return_value=5678, autospec=True)
def test_rate_multiple_months_rated(mock_token2, mock_email, client):
      # send data to signup using post method
    # redirects to email confirmation
    ...
    with client:
        ...
        assert br_rate.is_survey_completed == True
        assert br_rate.buddy_rate == 2
        # Checks if the reciver received points by the sender 
        assert br_rate.reward_points == 5
        # Checks if the sender received points for completing the survey 
        assert br_rate.sender_survey_points == 5
        
        assert response.request.path == '/rateconfirmation'
        assert b"You have successfully rated your buddy"  in response.data

        # Go back to buddy rate page and user tries to rate the same buddy on the same month the buddy star rating will not change and the rewards points will not change
        response = client.get(f"/rate?id={br.id}")
        br_id = int(request.args.get("id"))
        br = BuddyRelation.query.filter_by(id=br_id).first()
        user = User.query.filter_by(username=current_user.username).first()
        ...
        assert b"Subject for Buddy you are Rating:" in response.data
        assert br.study_interest.course.course_name in response.text
        response = client.post(f"/rate?id={br_id}", data={"month": 4,
                                                        "year": 2023,
                                                        "is_score_improved": "Yes",
                                                        "is_gained_knowledge": "Yes", 
                                                        "buddy_rate": 5,  
        ...
        # Go back to dashboard to check rewards points
        response = client.get("/dashboard")
        assert response.request.path == '/dashboard'
        user = User.query.filter_by(id=buddy_id).first()
        assert user.total_reward_points == 15
    ...
    # 1st user goes to dashboard and checks their connections and then rate the buddy
    response = client.get('/')
    assert response.request.path == '/'
    response = client.get('/login')
    assert response.request.path == '/login'
    with client:
        ...
        # Checking the buddy star rating and rewards points
        user = User.query.filter_by(id=buddy_id).first()
        assert user.total_reward_points == 15
