...
'''
This function tests if the the buddy receiver sees the pending invitation on the dashboard 
'''
...
def test_pending_invitation(mock_token2, mock_email, client):
    ...
    #1st USER checks their pending invitation requests 
    response = client.get('/')
    assert response.request.path == '/'
...
@mock.patch("app.extensions.email.send", return_value=True, autospec=True)
@mock.patch("random.randint", return_value=5678, autospec=True)
def test_accept_invitation(mock_token2, mock_email, client):
    # send data to signup using post method
    # redirects to email confirmation
    ...
    # 2nd user goes to dashboard and checks their connections
    response = client.get('/')
    assert response.request.path == '/'
    response = client.get('/login')
    assert response.request.path == '/login'
...
'''
This function tests if the the buddy receiver sees the pending invitation on the dashboard
and denies the invitation 
'''
@mock.patch("app.extensions.email.send", return_value=True, autospec=True)
@mock.patch("random.randint", return_value=5678, autospec=True)
def test_deny_invitation(mock_token2, mock_email, client):
    # send data to signup using post method
    # redirects to email confirmation
    ...
    #1st USER checks their pending invitation requests and clicks on the request and denies the request
    response = client.get('/')
    assert response.request.path == '/'
    response = client.get('/login')
    assert response.request.path == '/login'
    with client:
        ...
        assert br.sender.last_name in response.text
        assert br.study_interest.course.course_name in response.text
        response = client.post(f"/invitation?id={br.id}", data={"deny_buddy_but": "1"}, follow_redirects=True)
        assert response.status_code == 200
        br = BuddyRelation.query.filter_by(buddy_sender=user_sender.id, buddy_receiver=user_receiver.id, study_interest_id=si_receiver.id).first()
        assert br.invitation_status == 'D'
