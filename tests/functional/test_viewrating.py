...
'''
This function tests if the data is correctly displayed in the view ratings page
'''
...
def test_view_rating_success(mock_token2, mock_email, client):
    ...
    with client:
        ...
        ## Go to the viewratings page and the user should see the month and year of rating, the course, the buddy rating, who the rating wasd given by, and the comments given
        response = client.get('/viewratings')
        assert response.request.path == '/viewratings'
        user = User.query.filter_by(username=current_user.username).first()
        buddy_rating = BuddyRating.query.filter_by(rating_receiver=user.id).order_by(desc(BuddyRating.rating_date)).first()
        assert b"View Ratings" in response.data
        ...
        assert buddy_rating.sender.first_name in response.text
        assert buddy_rating.sender.last_name in response.text
        assert b"Comments" in response.data
        assert buddy_rating.comment in response.text
